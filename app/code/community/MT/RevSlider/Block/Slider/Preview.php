...
class MT_RevSlider_Block_Slider_Preview extends Mage_Core_Block_Template implements Mage_Widget_Block_Interface{
    ...
    protected function _construct(){
        ...
        $this->setData('cache_tags', array(MT_RevSlider_Model_Slider::CACHE_TAGS));
    }

    public function getCacheKey(){
        return MT_RevSlider_Model_Slider::CACHE_KEY_PREFIX . Mage::app()->getStore()->getId() . $this->getData('id');
    }
    ...
    /**
     * @param MT_RevSlider_Model_Slider $slider
        ...
        if ($slider && $this->numSlides){
            ...
            foreach ($slides as $slide){
                ...
                switch ($slide->getData('background_type')){
                    case 'trans':
                        $urlSlideImage = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/images/transparent.png';
                        break;
                    case 'solid':
                        $urlSlideImage = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/images/transparent.png';
                ...
                if ($slider->getData('lazy_load') == 'on'){
                    ...
                    $urlSlideImage = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/images/dummy.png';
                }
            ...
            switch ($type){
                ...
                case 'image':
                    ...
                    if ($lazyload == 'on') {
                        ...
                        $urlImage = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/images/dummy.png';
                    }
    ...
    public function renderJs($slider){
        ...
        $videoJsUrl = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/videojs/';
        ...
        if ($slider->getData('using_jquery') == 'true'){
            $scripts[] = Mage::getBaseUrl('js').'magenthemes/jquery/jquery.min.js';
        }
        $html .= '<script type="text/javascript">delete window["console"];</script>';
        $scripts[] = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/js/jquery.themepunch.tools.min.js?v=4.5.9';
        $scripts[] = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/js/jquery.themepunch.revolution.min.js?v=4.5.9';
        //$scripts[] = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/js/jquery.themepunch.revolution.js';
        foreach ($scripts as $script){
            $html .= "<script type='text/javascript' src='{$script}'></script>\r\n";
        }
        $styles = array();
        $styles[] = Mage::getBaseUrl('js').'mt/revslider/rs-plugin/css/settings.css?v=4.5.9';
